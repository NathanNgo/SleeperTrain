...
func add_edge(edge: GridRailwayEdge) -> void:
	var astar_from_id = vertex_id_to_astar_id_mapping[edge.first_point]
	var astar_to_id = vertex_id_to_astar_id_mapping[edge.last_point]
...
func remove_vertex(id: Vector2) -> void:
	...
	for vertex_connection in vertex_connections:
		var first_vertex_id: Vector2 = vertex_connection[Globals.FIRST_VERTEX_ID_IN_EDGE_ID]
		var second_vertex_id: Vector2 = vertex_connection[Globals.SECOND_VERTEX_ID_IN_EDGE_ID]
...
func remove_edge(id: Array[Vector2]) -> void:
	var first_vertex_id: Vector2 = id[Globals.FIRST_VERTEX_ID_IN_EDGE_ID]
	var second_vertex_id: Vector2 = id[Globals.SECOND_VERTEX_ID_IN_EDGE_ID]
	var edge: GridRailwayEdge = get_edge(id)

	if not edge:
		return

	var astar_from_id = vertex_id_to_astar_id_mapping[first_vertex_id]
	var astar_to_id = vertex_id_to_astar_id_mapping[second_vertex_id]
...
func get_vertex_by_name(vertex_name: String) -> GridRailwayVertex:
	if vertex_name in vertex_name_to_vertex_id_mapping:
		var id = vertex_name_to_vertex_id_mapping[vertex_name]
		return vertexes[id]
...
func get_edge(id: Array[Vector2]) -> GridRailwayEdge:
	var reverse_id = [id[Globals.SECOND_VERTEX_ID_IN_EDGE_ID], id[Globals.FIRST_VERTEX_ID_IN_EDGE_ID]]
...
func get_path(from_id: Vector2, to_id: Vector2) -> PackedVector2Array:
