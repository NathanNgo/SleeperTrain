...
func set_available_character_list(character_ids: Array[int]) -> void:
	for character_id in character_ids:
		...
		character_button.pressed.connect(_on_character_button_pressed.bind(character_data.id))

		_passengers_container.add_child(character_button)


func _on_minus_button_pressed() -> void:
	_carriage_selection = clamp(_carriage_selection - 1, MIN_CARRIAGES, total_carriages)
